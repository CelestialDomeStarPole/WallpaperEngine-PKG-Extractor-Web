...
在**用户浏览器本地**解析 Wallpaper Engine 的 `.pkg`（桌面）/ `.mpkg`（安卓）壁纸包，也支持单独的 `.tex` 贴图文件；提取 jpg / png / mp4 / webm 及包内任意资源；支持 `.tex` 解码转换与动画贴图重编码为 APNG / GIF；一切行为均在本地进行，与网站服务器无关，可静态部署。[示例网站](https://pkg.cdsp.us.ci)

## 功能

- 拖拽 / 选择 / Ctrl+V 粘贴 `.pkg` / `.mpkg` / `.tex` 文件，最大接近 2GB（解析只读目录表，包体不驻留内存）
- 条目列表 + 图片/视频缩略预览 + 大图弹窗
- `.tex → jpg/png/mp4` 自动转换（可关闭，关闭则原样导出 `.tex`）；视频纹理（TEX 里包着 mp4）零拷贝原样导出为 `.mp4`
- 纹理层级可选：默认只导最高分辨率，也可勾选同时导出 mip 链的低层级（文件名加 `.mipN` 后缀）
- 动画贴图（带帧表的 `.tex`）重编码为单个 **APNG**（无损、保留透明）或 **GIF**（256 色、体积小），也可两者都出；一帧颜色数 ≤255 时 GIF 用精确调色板，**逐像素无损**（平涂/矢量风格壁纸基本都满足）
- 多选卡片：选中文件一键下载 / 打包为 ZIP
- 单文件下载 / 保留目录结构的 ZIP 批量打包（流式写出，不把整包读进内存）
- 展示方式可切换：**按文件类型**归组（图片/视频/音乐/JSON/其他）或**按目录结构**浏览包内路径，选择会被记住；目录模式下面包屑逐级可点，左方向键退到上级目录
